...
def lift_reclaimed_two_block_carrier_to_1024_context(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.tie_embeddings or model_cfg.final_tail_smear_gate:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 0 or model_cfg.recurrence_loops != 0 or model_cfg.tail_layers != 3:
    ...
    cfg.quant.keep_float_name_patterns = (
        ...
        "tok_emb.weight",
        "tail.1.mlp.",
        "tail.1.attn.q_proj.weight",
    )


def add_top_layer_xsa_on_reclaimed_two_block_carrier(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.tie_embeddings or model_cfg.final_tail_smear_gate:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 0 or model_cfg.recurrence_loops != 0 or model_cfg.tail_layers != 2:
        return
    if model_cfg.mlp_mult != 2 or model_cfg.shared_mlp_hidden_bonus != 0:
        return
    expected_tail_bonuses = (model_cfg.d_model * 14, model_cfg.d_model * 7)
    if model_cfg.non_recurrent_mlp_hidden_bonus != model_cfg.d_model * 7:
        return
    if model_cfg.tail_mlp_hidden_bonuses != expected_tail_bonuses:
        return
    ...
    if model_cfg.fake_quant_start_step != cfg.train_seq_len_warmup_steps:
        return
    if model_cfg.seq_len != 1024:
        return
    if cfg.grad_accum_steps != 2:
        return
    if cfg.train_batch_tokens != 49_152 or cfg.val_batch_tokens != 122_880:
        return
    if cfg.train_seq_len_min != 768 or cfg.train_seq_len_warmup_steps != 160:
        return
    if cfg.optim.warmdown_steps != 80:
        return
    if cfg.quant.low_bit_bits != 6:
    ...
    if model_cfg.xsa_window_size > 0 or model_cfg.xsa_tail_layers > 0:
        return
    model_cfg.xsa_window_size = 64
    model_cfg.xsa_tail_layers = 1
    model_cfg.xsa_scale_init = 0.12
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    lift_reclaimed_two_block_carrier_to_1024_context(cfg)
    add_top_layer_xsa_on_reclaimed_two_block_carrier(cfg)
    return cfg
