...
def extend_warmup_and_align_shared_final_tail_mlp_on_tied_embed_final_tail_q_line(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.tie_embeddings:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 1 or model_cfg.tail_layers != 3:
    ...
    if any(pattern in cfg.quant.keep_float_name_patterns for pattern in new_patterns):
        return
    cfg.train_seq_len_warmup_steps = 200
    model_cfg.fake_quant_start_step = cfg.train_seq_len_warmup_steps
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    extend_warmup_and_align_shared_final_tail_mlp_on_tied_embed_final_tail_q_line(cfg)
    return cfg
