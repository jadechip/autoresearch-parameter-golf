...
def pair_attention_float_with_shared_and_final_tail_mlp_qat_relax_on_tied_embed_final_tail_q_line(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.tie_embeddings:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 1 or model_cfg.tail_layers != 3:
    ...
    if "tok_emb.weight" not in cfg.quant.keep_float_name_patterns:
        return
    attention_float_patterns = ("attn.q_proj", "attn.k_proj", "attn.v_proj", "attn.out_proj")
    if any(pattern in cfg.quant.keep_float_name_patterns for pattern in attention_float_patterns):
        return
    model_cfg.shared_mlp_fake_quant_during_train = False
    model_cfg.final_tail_mlp_fake_quant_during_train = False
    cfg.quant.keep_float_name_patterns = tuple(dict.fromkeys((*cfg.quant.keep_float_name_patterns, *attention_float_patterns)))
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    pair_attention_float_with_shared_and_final_tail_mlp_qat_relax_on_tied_embed_final_tail_q_line(cfg)
    return cfg
