...
def replace_branch_tip_neighbor_with_two_tail_smear_bigram_family(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if (
        not model_cfg.final_tail_neighbor_mixer
        or model_cfg.final_tail_smear_gate
        or model_cfg.smear_tail_layers > 0
        or model_cfg.tail_bigram_ranks is not None
        or not model_cfg.tie_embeddings
    ):
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 0 or model_cfg.recurrence_loops != 0 or model_cfg.tail_layers != 3:
        return
    ...
    model_cfg.smear_tail_layers = 2
    model_cfg.tail_bigram_ranks = (0, model_cfg.d_model // 8, model_cfg.d_model // 8)
    model_cfg.tail_mlp_hidden_bonuses = (
        expected_tail_bonuses[0],
...
class RecurrentGPT(nn.Module):
    def __init__(self, cfg: ModelConfig):
        ...
        smear_tail_start = cfg.tail_layers - cfg.smear_tail_layers
        ...
        self.tail = nn.ModuleList(
            [
                TransformerBlock(
                    ...
                    smear_gate=(
                        (cfg.final_tail_smear_gate and tail_idx == cfg.tail_layers - 1)
                        or (cfg.smear_tail_layers > 0 and tail_idx >= smear_tail_start)
                    ),
...
def validate_config(cfg: TrainConfig) -> None:
    ...
    if cfg.model.smear_tail_layers < 0:
        raise ConfigError("smear_tail_layers must be >= 0")
    if cfg.model.smear_tail_layers > cfg.model.tail_layers:
        raise ConfigError("smear_tail_layers must be <= tail_layers")
    ...
    if cfg.model.final_tail_smear_gate and cfg.model.smear_tail_layers > 0:
        raise ConfigError("final_tail_smear_gate cannot be combined with smear_tail_layers")
    if cfg.model.final_tail_neighbor_mixer and cfg.model.smear_tail_layers > 0:
        raise ConfigError("final_tail_neighbor_mixer cannot be combined with smear_tail_layers")
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    replace_branch_tip_neighbor_with_two_tail_smear_bigram_family(cfg)
