...
def reallocate_loop_adapter_capacity_away_from_k_on_accepted_deep_tail(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 2 or model_cfg.tail_layers != 7:
        return
    ...
    if not math.isclose(cfg.quant.clip_percentile, 96.5, rel_tol=0.0, abs_tol=1e-9):
        return
    model_cfg.adapter_rank = 12
    model_cfg.adapter_alpha = 24.0
    model_cfg.adapter_targets = ("q", "v", "attn_out", "mlp_in", "mlp_out")
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    reallocate_loop_adapter_capacity_away_from_k_on_accepted_deep_tail(cfg)
    return cfg
