...
def replace_branch_tip_multi_soup_with_depth12_ptq_carrier(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.final_tail_neighbor_mixer or not model_cfg.tie_embeddings or model_cfg.final_tail_smear_gate:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 0 or model_cfg.recurrence_loops != 0 or model_cfg.tail_layers != 3:
    ...
    model_cfg.non_recurrent_mlp_hidden_bonus = model_cfg.d_model // 2
    model_cfg.tail_mlp_hidden_bonuses = None
    model_cfg.shared_q_low_rank = model_cfg.d_model // 4
    model_cfg.tail_q_low_ranks = tuple(model_cfg.d_model // 4 for _ in range(model_cfg.tail_layers))
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    replace_branch_tip_multi_soup_with_depth12_ptq_carrier(cfg)
