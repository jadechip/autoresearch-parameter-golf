...
def keep_tail_attention_float_on_smear_gate_shared_low_rank_q_carrier(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.final_tail_smear_gate or not model_cfg.tie_embeddings:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 1 or model_cfg.tail_layers != 3:
    ...
    if not math.isclose(cfg.quant.clip_percentile, 96.5, rel_tol=0.0, abs_tol=1e-9):
        return
    cfg.quant.keep_float_name_patterns = tuple(
        dict.fromkeys((*cfg.quant.keep_float_name_patterns, *(f"tail.{idx}.attn." for idx in range(model_cfg.tail_layers))))
    )
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    keep_tail_attention_float_on_smear_gate_shared_low_rank_q_carrier(cfg)
    return cfg
