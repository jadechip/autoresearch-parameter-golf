...
def retune_optimizer_bundle_on_int6_stemless_deep_tail(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 2 or model_cfg.tail_layers != 8:
        return
    if model_cfg.mlp_mult != 2 or model_cfg.shared_mlp_hidden_bonus != (model_cfg.d_model * 3) // 8:
    ...
    if cfg.optim.warmdown_steps != 80:
        return
    cfg.optim.matrix_lr = 0.011
    cfg.optim.muon_momentum = 0.96
    cfg.optim.warmdown_steps = 100
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    retune_optimizer_bundle_on_int6_stemless_deep_tail(cfg)
    return cfg
