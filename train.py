...
def soften_train_fake_quant_clip_on_current_deep_tail(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 2 or model_cfg.tail_layers != 7:
        return
    ...
    if not math.isclose(model_cfg.fake_quant_clip_percentile, 100.0, rel_tol=0.0, abs_tol=1e-9):
        return
    model_cfg.fake_quant_clip_percentile = 99.5
...
def per_row_fake_quant_ste(weight: Tensor, clip_percentile: float = 100.0, eps: float = 1e-8) -> Tensor:
    ...
    if clip_percentile >= 100.0:
        ...
        if weight.ndim != 2:
            clip = torch.quantile(abs_weight.flatten(), q_level) if abs_weight.numel() else weight.new_tensor(0.0)
        else:
            clip = (
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    soften_train_fake_quant_clip_on_current_deep_tail(cfg)
    return cfg
