...
def reallocate_tail_q_rank_toward_later_blocks_on_three_block_carrier(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.tie_embeddings or model_cfg.final_tail_smear_gate:
        return
    ...
    model_cfg.tail_q_low_ranks = (model_cfg.d_model // 8, (model_cfg.d_model * 3) // 8, 0)
...
class RecurrentGPT(nn.Module):
    def __init__(self, cfg: ModelConfig):
        ...
        if tail_q_low_ranks is not None:
            if len(tail_q_low_ranks) != cfg.tail_layers:
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    reallocate_tail_q_rank_toward_later_blocks_on_three_block_carrier(cfg)
    return cfg
