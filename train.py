...
def switch_branch_tip_neighbor_carrier_to_dense_no_qat_triple_soup_selection(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.final_tail_neighbor_mixer or not model_cfg.tie_embeddings or model_cfg.final_tail_smear_gate:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 0 or model_cfg.recurrence_loops != 0 or model_cfg.tail_layers != 3:
    ...
    if cfg.checkpoint_every != 0 or cfg.checkpoint_archive_limit != 0 or cfg.post_quant_candidate_limit != 0:
        return
    if cfg.select_post_quant_best_checkpoint or cfg.post_quant_include_triple_soup:
        return
...
def score_post_quant_triple_soup_candidate(
    ...
    latest_resume_state = load_checkpoint(checkpoint_paths[-1], cfg, raw_model, opt_bundle, train_loader, lawa)
    ...
    candidate = score_post_quant_loaded_model_candidate(
        ...
        checkpoint_step=checkpoint_step,
        elapsed_training_seconds=latest_resume_state.elapsed_training_seconds,
        total_tokens=latest_resume_state.total_tokens,
        last_train_loss=latest_resume_state.last_train_loss,
        float_val=None,
        raw_model=raw_model,
        device=device,
        val_tokens=val_tokens,
...
def select_best_post_quant_checkpoint(
    ...
    if winner.source == "triple_soup":
        winner_payloads = [load_checkpoint_payload(path, cfg) for path in winner.checkpoint_paths]
        load_checkpoint(Path(winner.checkpoint_paths[-1]), cfg, raw_model, opt_bundle, train_loader, lawa)
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    switch_branch_tip_neighbor_carrier_to_dense_no_qat_triple_soup_selection(cfg)
    return cfg
