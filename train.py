...
class ModelConfig:
    ...
    penultimate_tail_meta_ttt_rank: int = 0
...
def add_penultimate_meta_ttt_on_front_loaded_branch_tip_carrier(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if model_cfg.penultimate_tail_meta_ttt_rank > 0:
        return
    if not model_cfg.final_tail_neighbor_mixer or not model_cfg.tie_embeddings or model_cfg.final_tail_smear_gate:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 0 or model_cfg.recurrence_loops != 0 or model_cfg.tail_layers != 3:
    ...
    model_cfg.penultimate_tail_meta_ttt_rank = model_cfg.d_model // 32
    cfg.quant.keep_float_name_patterns = (
        "norm",
        "scale",
        "gain",
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        self.meta_ttt = AdaptiveCausalMetaTTT(cfg, rank=meta_ttt_rank) if meta_ttt_rank > 0 else None
        self.meta_ttt_scale = nn.Parameter(torch.ones(scale_shape)) if meta_ttt_rank > 0 else None
    ...
    def set_global_step(self, step: int) -> None:
        ...
        if self.meta_ttt is not None:
            self.meta_ttt.set_global_step(step)
    ...
    def forward(self, x: Tensor, slot: int | None = None) -> Tensor:
        ...
        if self.meta_ttt_scale is not None:
            assert self.meta_ttt is not None
            x = x + self.meta_ttt(x) * self.residual_scale(self.meta_ttt_scale, x, slot)
...
class AdaptiveCausalMetaTTT(nn.Module):
    def __init__(self, cfg: ModelConfig, *, rank: int):
        ...
        self.write_gate_proj = FakeQuantLinear(
            cfg.d_model,
            1,
            bias=False,
            fake_quant_during_train=cfg.fake_quant_during_train,
    ...
    def set_global_step(self, step: int) -> None:
        ...
        self.k_proj.set_global_step(step)
        self.v_proj.set_global_step(step)
        self.write_gate_proj.set_global_step(step)
        self.out_proj.set_global_step(step)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        v = torch.tanh(self.v_proj(h))
        write = torch.sigmoid(self.write_gate_proj(h))
        updates = torch.einsum("bti,btj->btij", v * write, k)
        prefix_memory = torch.cumsum(updates, dim=1) - updates
        counts = torch.cumsum(write, dim=1) - write
        mixed = torch.einsum("btij,btj->bti", prefix_memory / counts.clamp_min(1.0).unsqueeze(-1), q)
        ...
        self.tail = nn.ModuleList(
            [
                TransformerBlock(
                    ...
                    meta_ttt_rank=cfg.penultimate_tail_meta_ttt_rank if tail_idx == cfg.tail_layers - 2 else 0,
                )
...
def validate_config(cfg: TrainConfig) -> None:
    ...
    if cfg.model.penultimate_tail_meta_ttt_rank < 0:
        raise ConfigError("penultimate_tail_meta_ttt_rank must be >= 0")
    if cfg.model.penultimate_tail_meta_ttt_rank >= cfg.model.d_model:
        raise ConfigError("penultimate_tail_meta_ttt_rank must be smaller than d_model")
    ...
    if cfg.model.penultimate_tail_meta_ttt_rank > 0 and cfg.model.tail_layers < 2:
        raise ConfigError("penultimate_tail_meta_ttt_rank requires at least 2 tail layers")
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    add_penultimate_meta_ttt_on_front_loaded_branch_tip_carrier(cfg)
