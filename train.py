...
class ModelConfig:
    ...
    factorized_history_head: bool = False
...
def replace_branch_tip_neighbor_with_factorized_history_head(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.final_tail_neighbor_mixer or model_cfg.final_tail_smear_gate or model_cfg.factorized_history_head:
        return
    if not model_cfg.tie_embeddings:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 0 or model_cfg.recurrence_loops != 0 or model_cfg.tail_layers != 3:
    ...
    if not math.isclose(cfg.quant.clip_percentile, 96.5, rel_tol=0.0, abs_tol=1e-9):
        return
    model_cfg.final_tail_neighbor_mixer = False
    model_cfg.factorized_history_head = True
    model_cfg.tail_mlp_hidden_bonuses = (
        expected_tail_bonuses[0],
        expected_tail_bonuses[1],
...
class FactorizedHistoryHead(nn.Module):
    def __init__(self, cfg: ModelConfig):
        super().__init__()
        exact_rank = max(64, cfg.d_model * 3 // 8)
        hash_rank = max(32, cfg.d_model * 3 // 32)
        self.num_buckets = max(cfg.vocab_size * 2, 512)
        self.exact_table = nn.Embedding(cfg.vocab_size, exact_rank)
    ...
    def forward(self, input_ids: Tensor, dtype: torch.dtype) -> Tensor:
        logits = self.exact_proj(self.exact_table(input_ids)).to(dtype=dtype)
...
class RecurrentGPT(nn.Module):
    def __init__(self, cfg: ModelConfig):
        ...
        self.factorized_history_head = FactorizedHistoryHead(cfg) if cfg.factorized_history_head else None
    ...
    def set_global_step(self, step: int) -> None:
        if self.factorized_history_head is not None:
            self.factorized_history_head.set_global_step(step)
        if self.lm_head is not None:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if self.factorized_history_head is not None:
            logits = logits + self.factorized_history_head(input_ids, logits.dtype)
...
def validate_config(cfg: TrainConfig) -> None:
    ...
    if cfg.model.factorized_history_head and (cfg.model.final_tail_smear_gate or cfg.model.final_tail_neighbor_mixer):
        raise ConfigError("factorized_history_head cannot be combined with final-tail local mixers")
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    replace_branch_tip_neighbor_with_factorized_history_head(cfg)
