...
def pair_in_band_attention_and_shared_carrier_float_with_shared_and_final_tail_mlp_qat_relaxation_on_tied_embed_line(
    cfg: TrainConfig,
) -> None:
    model_cfg = cfg.model
    if not model_cfg.tie_embeddings:
    ...
    if "tok_emb.weight" not in cfg.quant.keep_float_name_patterns:
        return
    new_patterns = ("attn.q_proj", "attn.k_proj", "attn.v_proj", "attn.out_proj", "shared.0.mlp.")
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    pair_in_band_attention_and_shared_carrier_float_with_shared_and_final_tail_mlp_qat_relaxation_on_tied_embed_line(cfg)
    return cfg
