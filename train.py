...
class ModelConfig:
    ...
    non_recurrent_mlp_fake_quant_during_train: bool | None = None
...
def disable_non_recurrent_mlp_fake_quant_on_late_mlp_qat_small_batch_line(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 1 or model_cfg.tail_layers != 3:
        return
    if model_cfg.mlp_mult != 2 or model_cfg.shared_mlp_hidden_bonus != model_cfg.d_model:
    ...
    if not model_cfg.fake_quant_during_train or model_cfg.attn_fake_quant_during_train is not False:
        return
    if model_cfg.non_recurrent_mlp_fake_quant_during_train is not None:
        return
    ...
    if cfg.train_seq_len_min != 640 or cfg.train_seq_len_warmup_steps != 160:
        return
    model_cfg.non_recurrent_mlp_fake_quant_during_train = False
...
class ReLU2MLP(nn.Module):
    def __init__(
        ...
        mlp_fake_quant_during_train = cfg.fake_quant_during_train if fake_quant_during_train is None else fake_quant_during_train
        ...
        self.stem = nn.ModuleList(
            [
                TransformerBlock(
                    ...
                    mlp_fake_quant_during_train=cfg.non_recurrent_mlp_fake_quant_during_train,
                )
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    disable_non_recurrent_mlp_fake_quant_on_late_mlp_qat_small_batch_line(cfg)
    return cfg
