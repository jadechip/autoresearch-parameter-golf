...
def slightly_widen_non_recurrent_mlp_on_later_calibrated_deep_tail(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    target_bonus = (model_cfg.d_model * 17) // 32
    if model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 2 or model_cfg.tail_layers != 7:
        return
    if model_cfg.mlp_mult != 2 or model_cfg.shared_mlp_hidden_bonus != (model_cfg.d_model * 7) // 16:
        return
    if model_cfg.non_recurrent_mlp_hidden_bonus not in {0, model_cfg.d_model // 2, target_bonus}:
        return
    if model_cfg.adapter_rank != 8 or tuple(model_cfg.adapter_targets) != ALLOWED_ADAPTER_TARGETS:
        return
    if model_cfg.fake_quant_start_step != 20:
    ...
    if not math.isclose(cfg.quant.clip_percentile, 96.5, rel_tol=0.0, abs_tol=1e-9):
        return
    model_cfg.non_recurrent_mlp_hidden_bonus = target_bonus
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    slightly_widen_non_recurrent_mlp_on_later_calibrated_deep_tail(cfg)
    return cfg
