...
def narrow_gqa_and_keep_final_tail_block_float_on_final_tail_q_small_batch_line(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.tie_embeddings:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 1 or model_cfg.tail_layers != 3:
    ...
    if cfg.train_seq_len_min != 640 or cfg.train_seq_len_warmup_steps != 160:
        return
    final_tail_float_patterns = (
        "tail.2.attn.q_proj",
        "tail.2.attn.k_proj",
        "tail.2.attn.v_proj",
        "tail.2.attn.out_proj",
        "tail.2.mlp.fc.weight",
        "tail.2.mlp.proj.weight",
    )
    if any(pattern in cfg.quant.keep_float_name_patterns for pattern in final_tail_float_patterns):
        return
    model_cfg.num_kv_heads = 2
    cfg.quant.keep_float_name_patterns = (*cfg.quant.keep_float_name_patterns, *final_tail_float_patterns)
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    narrow_gqa_and_keep_final_tail_block_float_on_final_tail_q_small_batch_line(cfg)
    return cfg
