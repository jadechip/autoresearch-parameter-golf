...
RESUME_HASH_EXCLUDED_KEYS = HASH_EXCLUDED_KEYS | {
    ...
    "checkpoint_archive_warmdown_only",
    "post_quant_candidate_limit",
    "select_post_quant_best_checkpoint",
    "post_quant_include_pairwise_soup",
    "post_quant_include_triple_soup",
...
class ModelConfig:
    ...
    tail_mlp_hidden_bonuses: tuple[int, ...] | None = None
...
class TrainConfig:
    ...
    post_quant_include_pairwise_soup: bool = False
    post_quant_include_triple_soup: bool = False
...
def restore_shared_full_rank_q_and_keep_shared_block_float_on_tied_embed_line(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.tie_embeddings:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 1 or model_cfg.tail_layers != 3:
    ...
    if tuple(cfg.quant.low_bit_name_patterns) != ("mlp.fc.weight", "mlp.proj.weight"):
        return
    expected_keep_float_patterns = ("norm", "scale", "gain", "adapter", "lm_head", "tok_emb.weight")
    ...
    if cfg.train_seq_len_min != 640 or cfg.train_seq_len_warmup_steps != 160:
        return
    model_cfg.shared_q_low_rank = 0
    cfg.quant.keep_float_name_patterns = tuple(
        dict.fromkeys((*cfg.quant.keep_float_name_patterns, "shared.0.attn.", "shared.0.mlp."))
    )
...
def restore_shared_low_rank_q_on_smear_gate_shared_float_carrier(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
...
def reallocate_shared_budget_into_near_cap_final_tail_carrier(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.final_tail_smear_gate or not model_cfg.tie_embeddings:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 1 or model_cfg.tail_layers != 3:
    ...
    if not math.isclose(cfg.quant.clip_percentile, 96.5, rel_tol=0.0, abs_tol=1e-9):
        return
    tail_idx = model_cfg.tail_layers - 1
    ...
    cfg.quant.keep_float_name_patterns = (
        ...
        "tok_emb.weight",
        f"tail.{tail_idx}.mlp.",
        f"tail.{tail_idx}.attn.q_proj.weight",
    )
...
def flatten_near_cap_carrier_into_four_unique_blocks(cfg: TrainConfig) -> None:
    ...
    if not math.isclose(cfg.quant.clip_percentile, 96.5, rel_tol=0.0, abs_tol=1e-9):
        return
    model_cfg.shared_layers = 0
    model_cfg.recurrence_loops = 0
    model_cfg.tail_layers = 4
    model_cfg.shared_mlp_hidden_bonus = 0
    model_cfg.non_recurrent_mlp_hidden_bonus = model_cfg.d_model * 19 // 4
    cfg.quant.keep_float_name_patterns = (
        "norm",
        "scale",
        "gain",
...
def trade_one_four_block_layer_for_three_wider_unique_blocks(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.tie_embeddings or model_cfg.final_tail_smear_gate:
        return
    ...
    if not math.isclose(cfg.quant.clip_percentile, 96.5, rel_tol=0.0, abs_tol=1e-9):
        return
    model_cfg.tail_layers = 3
    model_cfg.non_recurrent_mlp_hidden_bonus = model_cfg.d_model * 7
    cfg.quant.keep_float_name_patterns = (
        "norm",
        "scale",
        "gain",
...
def front_load_tail_mlp_width_on_three_block_near_cap_carrier(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.tie_embeddings or model_cfg.final_tail_smear_gate:
        return
    ...
    if model_cfg.non_recurrent_mlp_hidden_bonus != model_cfg.d_model * 7:
        return
    if model_cfg.tail_mlp_hidden_bonuses is not None:
        return
    if model_cfg.q_low_rank != model_cfg.d_model // 4:
        return
    if model_cfg.shared_q_low_rank is not None or model_cfg.final_tail_q_low_rank != 0:
...
def restore_short_to_full_context_on_branch_tip_three_block_carrier(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.tie_embeddings or model_cfg.final_tail_smear_gate:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 0 or model_cfg.recurrence_loops != 0 or model_cfg.tail_layers != 3:
...
def move_full_rank_q_forward_and_compress_final_q_on_branch_tip_carrier(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.final_tail_neighbor_mixer or not model_cfg.tie_embeddings or model_cfg.final_tail_smear_gate:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 0 or model_cfg.recurrence_loops != 0 or model_cfg.tail_layers != 3:
    ...
    model_cfg.tail_mlp_hidden_bonuses = (
        ...
        expected_tail_bonuses[2] + model_cfg.d_model // 8,
    )
    cfg.quant.keep_float_name_patterns = (
        "norm",
        "scale",
...
def reallocate_tail_q_budget_into_four_block_depth_on_branch_tip_carrier(cfg: TrainConfig) -> None:
    ...
    model_cfg.final_tail_q_low_rank = None
    model_cfg.tail_q_low_ranks = (model_cfg.d_model // 8, model_cfg.d_model * 3 // 16, model_cfg.d_model * 3 // 16, 0)
    cfg.quant.keep_float_name_patterns = (
        "norm",
        "scale",
        "gain",
...
def switch_branch_tip_neighbor_carrier_to_dense_late_qat_multi_soup_selection(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.final_tail_neighbor_mixer or not model_cfg.tie_embeddings or model_cfg.final_tail_smear_gate:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 0 or model_cfg.recurrence_loops != 0 or model_cfg.tail_layers != 3:
...
def replace_branch_tip_neighbor_carrier_with_pre_final_xsa_canon_twelve_layer_branch(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if model_cfg.canon_rank > 0 or model_cfg.xsa_window_size > 0 or model_cfg.xsa_tail_layers > 0:
        return
    if not model_cfg.final_tail_neighbor_mixer or not model_cfg.tie_embeddings or model_cfg.final_tail_smear_gate:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 0 or model_cfg.recurrence_loops != 0 or model_cfg.tail_layers != 3:
    ...
    model_cfg.stem_canon_layers = None
    model_cfg.shared_canon_layers = None
    model_cfg.tail_canon_layers = (model_cfg.tail_layers - 2,)
    model_cfg.xsa_window_size = 128
    model_cfg.xsa_tail_layers = 1
    model_cfg.xsa_scale_init = 0.06
    model_cfg.fake_quant_start_step = cfg.train_seq_len_warmup_steps
    ...
    cfg.quant.keep_float_name_patterns = (
        ...
        "tok_emb.weight",
        "tail.5.mlp.",
        "tail.5.attn.q_proj.weight",
        "tail.5.attn.out_proj.weight",
    )
...
class GroupedQueryAttention(nn.Module):
    ...
    def xsa_window_summaries(self, x: Tensor) -> Tensor:
        if self.xsa_window_size <= 0:
            raise RuntimeError("xsa_window_summaries called with xsa disabled")
        bsz, heads, seqlen, head_dim = x.shape
        pad = (-seqlen) % self.xsa_window_size
        if pad:
            x = F.pad(x, (0, 0, 0, pad))
        num_windows = x.size(2) // self.xsa_window_size
        summary = x.reshape(bsz, heads, num_windows, self.xsa_window_size, head_dim).mean(dim=3)
        zero_summary = torch.zeros((bsz, heads, 1, head_dim), device=x.device, dtype=x.dtype)
        return torch.cat((zero_summary, summary), dim=2)

    def xsa_mask(self, seqlen: int, num_windows: int, device: torch.device, dtype: torch.dtype) -> Tensor:
        if self.xsa_window_size <= 0:
            raise RuntimeError("xsa_mask called with xsa disabled")
        token_windows = torch.arange(seqlen, device=device) // self.xsa_window_size
        summary_indices = torch.arange(num_windows + 1, device=device)
        allowed = summary_indices.unsqueeze(0) <= token_windows.unsqueeze(1)
        mask = torch.zeros((1, 1, seqlen, num_windows + 1), device=device, dtype=dtype)
        return mask.masked_fill(~allowed.view(1, 1, seqlen, num_windows + 1), torch.finfo(dtype).min)
    ...
    def forward(self, x: Tensor, slot: int | None = None) -> Tensor:
        ...
        if self.xsa_scale is not None and seqlen > self.xsa_window_size:
            summary_k = self.xsa_window_summaries(k)
            summary_v = self.xsa_window_summaries(v)
            summary_mask = self.xsa_mask(seqlen, summary_k.size(2) - 1, x.device, q.dtype)
            xsa = F.scaled_dot_product_attention(q, summary_k, summary_v, attn_mask=summary_mask, is_causal=False)
...
class RecurrentGPT(nn.Module):
    def __init__(self, cfg: ModelConfig):
        ...
        tail_hidden_bonuses = cfg.tail_mlp_hidden_bonuses
        if tail_hidden_bonuses is not None and len(tail_hidden_bonuses) != cfg.tail_layers:
        ...
        self.tail = nn.ModuleList(
            [
                TransformerBlock(
                    ...
                    q_low_rank=tail_q_low_ranks[tail_idx] if tail_q_low_ranks is not None else (
                        final_tail_q_low_rank if tail_idx == cfg.tail_layers - 1 else None
                    ),
                    xsa_window_size=cfg.xsa_window_size if tail_idx >= cfg.tail_layers - cfg.xsa_tail_layers else 0,
                    mlp_fake_quant_during_train=(
...
def checkpoint_candidate_label(path: Path) -> str:
...
def load_checkpoint_payload(path: str | Path, cfg: TrainConfig) -> dict[str, Any]:
...
def resume_state_from_checkpoint_payload(payload: Mapping[str, Any], path: str | Path) -> ResumeState:
...
def average_model_state_dicts(state_dicts: list[Mapping[str, Tensor]]) -> OrderedDict[str, Tensor]:
    if not state_dicts:
        raise ConfigError("checkpoint soup requires at least one model_state")
    ...
    for name in names:
        ...
        if ref.is_floating_point():
            ...
            for tensor in tensors:
                acc.add_(tensor.to(torch.float32))
            averaged[name] = (acc / len(tensors)).to(dtype=ref.dtype)
        else:
            averaged[name] = ref.clone()
    return averaged
...
def load_checkpoint(
    ...
    payload = load_checkpoint_payload(path, cfg)
    ...
    return resume_state_from_checkpoint_payload(payload, path)


def score_post_quant_loaded_model_candidate(
    cfg: TrainConfig,
    output_dir: Path,
    run_id: str,
    cfg_hash: str,
    ...
    return PostQuantCandidate(
        ...
        total_tokens=total_tokens,
        last_train_loss=last_train_loss,
        float_val_loss=None if float_val is None else float_val.val_loss,
        float_val_bpb=None if float_val is None else float_val.val_bpb,
...
def score_post_quant_checkpoint_candidate(
    ...
    candidate = score_post_quant_loaded_model_candidate(
        cfg=cfg,
        ...
        last_train_loss=resume_state.last_train_loss,
        float_val=resume_state.last_val,
        raw_model=raw_model,
        device=device,
        val_tokens=val_tokens,
        base_bytes_lut=base_bytes_lut,
...
def score_post_quant_pairwise_soup_candidate(
    cfg: TrainConfig,
    output_dir: Path,
    run_id: str,
    cfg_hash: str,
    ...
    payloads = [load_checkpoint_payload(path, cfg) for path in checkpoint_paths]
    latest_index = max(range(len(payloads)), key=lambda index: int(payloads[index]["next_step"]))
    latest_payload = payloads[latest_index]
    latest_resume_state = load_checkpoint(checkpoint_paths[latest_index], cfg, raw_model, opt_bundle, train_loader, lawa)
    raw_model.load_state_dict(average_model_state_dicts([payload["model_state"] for payload in payloads]), strict=True)
    checkpoint_step = max(int(payload["next_step"]) for payload in payloads)
    candidate = score_post_quant_loaded_model_candidate(
        cfg=cfg,
        output_dir=output_dir,
        run_id=run_id,
        cfg_hash=cfg_hash,
        candidate_id=candidate_id,
        source="pairwise_soup",
        checkpoint_paths=checkpoint_paths,
        checkpoint_step=checkpoint_step,
        elapsed_training_seconds=float(latest_payload["elapsed_training_seconds"]),
        total_tokens=int(latest_payload["total_tokens"]),
        last_train_loss=latest_payload["last_train_loss"],
        float_val=None,
        raw_model=raw_model,
        device=device,
        val_tokens=val_tokens,
    ...
    return candidate, latest_resume_state


def score_post_quant_triple_soup_candidate(
    cfg: TrainConfig,
    output_dir: Path,
    run_id: str,
    cfg_hash: str,
...
def select_best_post_quant_checkpoint(
    ...
    final_resume_state = load_checkpoint(final_checkpoint_path, cfg, raw_model, opt_bundle, train_loader, lawa)
    final_step = final_resume_state.next_step
    archived_paths = [
        path
        for path in list_archived_checkpoints(output_dir)
        if parse_archived_checkpoint_step(path) not in {None, final_step}
    ]
    ...
    for path in archived_paths:
        ...
        if step is None:
            continue
        candidate_id = f"archived-step-{step:06d}"
    ...
    load_checkpoint(final_checkpoint_path, cfg, raw_model, opt_bundle, train_loader, lawa)
    final_candidate = score_post_quant_loaded_model_candidate(
        cfg=cfg,
        output_dir=output_dir,
        run_id=run_id,
        cfg_hash=cfg_hash,
        candidate_id="final",
        source="final",
        checkpoint_paths=(final_checkpoint_path,),
        checkpoint_step=final_resume_state.next_step,
        elapsed_training_seconds=final_resume_state.elapsed_training_seconds,
        total_tokens=final_resume_state.total_tokens,
        last_train_loss=final_resume_state.last_train_loss,
        float_val=final_resume_state.last_val,
        raw_model=raw_model,
        device=device,
        val_tokens=val_tokens,
        base_bytes_lut=base_bytes_lut,
    ...
    candidate_records.append(final_candidate)
    candidate_resume_states["final"] = final_resume_state

    if cfg.post_quant_include_pairwise_soup and archived_paths:
        soup_base = archived_paths[-1]
        soup_id = f"{checkpoint_candidate_label(soup_base)}-plus-final-soup"
        soup_candidate, soup_resume_state = score_post_quant_pairwise_soup_candidate(
            cfg=cfg,
            output_dir=output_dir,
            run_id=run_id,
            cfg_hash=cfg_hash,
            checkpoint_paths=(soup_base, final_checkpoint_path),
            candidate_id=soup_id,
            raw_model=raw_model,
            opt_bundle=opt_bundle,
            train_loader=train_loader,
            lawa=lawa,
        ...
        candidate_records.append(soup_candidate)
        candidate_resume_states[soup_candidate.candidate_id] = soup_resume_state

    if cfg.post_quant_include_triple_soup and len(archived_paths) >= 2:
        soup_paths = (archived_paths[-2], archived_paths[-1], final_checkpoint_path)
        soup_candidate, soup_resume_state = score_post_quant_triple_soup_candidate(
            cfg=cfg,
            output_dir=output_dir,
            run_id=run_id,
            cfg_hash=cfg_hash,
            checkpoint_paths=soup_paths,
            candidate_id="last-two-archived-plus-final-soup",
            raw_model=raw_model,
            opt_bundle=opt_bundle,
            train_loader=train_loader,
            lawa=lawa,
    ...
    if winner.source in {"pairwise_soup", "triple_soup"}:
        winner_payloads = [load_checkpoint_payload(path, cfg) for path in winner.checkpoint_paths]
        latest_index = max(range(len(winner_payloads)), key=lambda index: int(winner_payloads[index]["next_step"]))
        load_checkpoint(Path(winner.checkpoint_paths[latest_index]), cfg, raw_model, opt_bundle, train_loader, lawa)
        raw_model.load_state_dict(average_model_state_dicts([payload["model_state"] for payload in winner_payloads]), strict=True)
        ...
        load_checkpoint(Path(winner.checkpoint_paths[0]), cfg, raw_model, opt_bundle, train_loader, lawa)
    ...
    atomic_write_json(
        output_dir / "post_quant_selection.json",
        {
            ...
            "selected_checkpoint_paths": list(winner.checkpoint_paths),
...
def validate_config(cfg: TrainConfig) -> None:
    ...
    if cfg.model.tail_mlp_hidden_bonuses is not None:
        if len(cfg.model.tail_mlp_hidden_bonuses) != cfg.model.tail_layers:
            raise ConfigError("tail_mlp_hidden_bonuses must have exactly one entry per tail layer")
        for hidden_bonus in cfg.model.tail_mlp_hidden_bonuses:
            if hidden_bonus < 0:
                raise ConfigError("tail_mlp_hidden_bonuses entries must be >= 0")
    ...
    if cfg.model.xsa_tail_layers < 0:
        raise ConfigError("xsa_tail_layers must be >= 0")
    if cfg.model.xsa_tail_layers > cfg.model.tail_layers:
        raise ConfigError("xsa_tail_layers must be <= tail_layers")
    if cfg.model.xsa_window_size > 0 and cfg.model.xsa_tail_layers <= 0:
        raise ConfigError("xsa_window_size requires xsa_tail_layers > 0")
    if cfg.model.xsa_tail_layers > 0 and cfg.model.xsa_window_size == 0:
        raise ConfigError("xsa_tail_layers requires xsa_window_size > 0")
    ...
    if cfg.post_quant_include_pairwise_soup and not cfg.select_post_quant_best_checkpoint:
        raise ConfigError("post_quant_include_pairwise_soup requires select_post_quant_best_checkpoint")
    if cfg.post_quant_include_pairwise_soup and cfg.checkpoint_archive_limit <= 0:
        raise ConfigError("post_quant_include_pairwise_soup requires checkpoint_archive_limit > 0")
    if cfg.post_quant_include_triple_soup and not cfg.select_post_quant_best_checkpoint:
        raise ConfigError("post_quant_include_triple_soup requires select_post_quant_best_checkpoint")
    if cfg.post_quant_include_triple_soup and cfg.checkpoint_archive_limit < 2:
        raise ConfigError("post_quant_include_triple_soup requires checkpoint_archive_limit >= 2")
        ...
        for step in range(start_step, target_iterations):
            ...
            if rank0 and cfg.checkpoint_every > 0 and completed_steps % cfg.checkpoint_every == 0:
                ...
                if should_archive_checkpoint(cfg, completed_steps, schedule_total_steps):
                    save_checkpoint(
                        archived_checkpoint_path(output_dir, completed_steps),
                        cfg,
                        run_id,
                        cfg_hash,
                        raw_model,
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    replace_branch_tip_neighbor_carrier_with_pre_final_xsa_canon_twelve_layer_branch(cfg)
    return cfg
