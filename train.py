...
def switch_branch_tip_neighbor_carrier_to_late_qat_checkpoint_soup_selection(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.final_tail_neighbor_mixer or not model_cfg.tie_embeddings or model_cfg.final_tail_smear_gate:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 0 or model_cfg.recurrence_loops != 0 or model_cfg.tail_layers != 3:
    ...
    if cfg.checkpoint_every != 0 or cfg.checkpoint_archive_limit != 0 or cfg.post_quant_candidate_limit != 0:
        return
    if cfg.select_post_quant_best_checkpoint or cfg.post_quant_include_pairwise_soup:
        return
    model_cfg.fake_quant_start_step = 1_024
    cfg.checkpoint_every = 32
    cfg.checkpoint_archive_limit = 1
    cfg.checkpoint_archive_warmdown_only = True
    cfg.post_quant_candidate_limit = 1
...
def average_model_state_dicts(state_dicts: Sequence[Mapping[str, Tensor]]) -> OrderedDict[str, Tensor]:
    if not state_dicts:
...
def select_best_post_quant_checkpoint(
    ...
    final_candidate, final_resume_state = score_post_quant_checkpoint_candidate(
        cfg=cfg,
        output_dir=output_dir,
        run_id=run_id,
        cfg_hash=cfg_hash,
    ...
    if cfg.post_quant_include_pairwise_soup and archived_paths:
        ...
        soup_step = parse_archived_checkpoint_step(soup_base)
        soup_id = "last-archived-plus-final-soup" if soup_step is None else f"step-{soup_step:06d}-plus-final-soup"
        ...
        candidate_resume_states[soup_id] = soup_resume_state
    if not candidate_records:
        raise ConfigError("post-quant checkpoint selection requires at least one candidate")
    ...
    if winner.source == "pairwise_soup":
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    switch_branch_tip_neighbor_carrier_to_late_qat_checkpoint_soup_selection(cfg)
