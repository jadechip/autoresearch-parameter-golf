...
class ModelConfig:
    ...
    final_tail_prev_token_mix: bool = False
...
def add_final_tail_prev_token_mix_on_near_cap_carrier(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if model_cfg.final_tail_prev_token_mix or not model_cfg.tie_embeddings:
        return
    if model_cfg.final_tail_smear_gate:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 1 or model_cfg.tail_layers != 3:
    ...
    if not math.isclose(cfg.quant.clip_percentile, 96.5, rel_tol=0.0, abs_tol=1e-9):
        return
    model_cfg.final_tail_prev_token_mix = True
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        self.prev_token_scale = nn.Parameter(torch.zeros(scale_shape)) if prev_token_mix else None
    ...
    def previous_token(self, x: Tensor) -> Tensor:
        if x.size(1) <= 1:
            return torch.zeros_like(x)
    ...
    def forward(self, x: Tensor, slot: int | None = None) -> Tensor:
        ...
        if self.prev_token_scale is not None:
            # Cheap bigram-style residual bias for the chosen local-module branch.
            x = x + self.previous_token(x) * self.residual_scale(self.prev_token_scale, x, slot)
        return x
        ...
        self.tail = nn.ModuleList(
            [
                TransformerBlock(
                    ...
                    prev_token_mix=cfg.final_tail_prev_token_mix and tail_idx == cfg.tail_layers - 1,
                )
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    add_final_tail_prev_token_mix_on_near_cap_carrier(cfg)
    return cfg
