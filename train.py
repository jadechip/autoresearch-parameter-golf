...
def keep_q_and_attn_out_float_on_tied_embed_final_tail_q_line(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.tie_embeddings:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 1 or model_cfg.tail_layers != 3:
    ...
    if "tok_emb.weight" not in cfg.quant.keep_float_name_patterns:
        return
    if "attn.q_proj" in cfg.quant.keep_float_name_patterns or "attn.out_proj.weight" in cfg.quant.keep_float_name_patterns:
        return
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    keep_q_and_attn_out_float_on_tied_embed_final_tail_q_line(cfg)
    return cfg
