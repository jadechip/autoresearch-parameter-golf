...
def keep_attention_float_on_tied_embedding_final_tail_q_line(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.tie_embeddings:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 1 or model_cfg.tail_layers != 3:
    ...
    if all(pattern in cfg.quant.keep_float_name_patterns for pattern in attention_float_patterns):
        return
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    keep_attention_float_on_tied_embedding_final_tail_q_line(cfg)
    return cfg
