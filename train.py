...
def align_quant_timing_and_clip_on_wider_deep_tail(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 2 or model_cfg.tail_layers != 7:
        return
    ...
    if not math.isclose(cfg.quant.clip_percentile, 96.5, rel_tol=0.0, abs_tol=1e-9):
        return
    model_cfg.fake_quant_start_step = 10
    cfg.quant.clip_percentile = 96.0
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    align_quant_timing_and_clip_on_wider_deep_tail(cfg)
    return cfg
