...
def restore_penultimate_tail_full_rank_q_on_smear_gate_low_rank_q_carrier(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.final_tail_smear_gate or not model_cfg.tie_embeddings:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 1 or model_cfg.tail_layers != 3:
    ...
    if model_cfg.q_low_rank != model_cfg.d_model // 4:
        return
    if model_cfg.shared_q_low_rank is not None or model_cfg.penultimate_tail_q_low_rank is not None:
        return
    if model_cfg.final_tail_q_low_rank != 0:
        return
    if model_cfg.final_tail_mlp_fake_quant_during_train is not None:
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    restore_penultimate_tail_full_rank_q_on_smear_gate_low_rank_q_carrier(cfg)
    return cfg
