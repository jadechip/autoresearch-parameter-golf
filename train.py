...
def align_final_tail_block_train_and_export_precision_on_tied_embed_final_tail_q_line(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.tie_embeddings:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 1 or model_cfg.tail_layers != 3:
    ...
    new_patterns = (
        f"tail.{tail_idx}.attn.q_proj",
        f"tail.{tail_idx}.attn.out_proj",
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    align_final_tail_block_train_and_export_precision_on_tied_embed_final_tail_q_line(cfg)
    return cfg
