...
class ModelConfig:
    ...
    tail_mlp_hidden_bonus_step: int = 0
...
def tilt_tail_mlp_width_toward_final_block_on_final_tail_q_small_batch_line(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.tie_embeddings:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 1 or model_cfg.tail_layers != 3:
    ...
    if model_cfg.non_recurrent_mlp_hidden_bonus != model_cfg.d_model * 6:
        return
    if model_cfg.tail_mlp_hidden_bonus_step != 0:
        return
    if model_cfg.q_low_rank != model_cfg.d_model // 4:
        return
    if model_cfg.final_tail_q_low_rank != 0:
    ...
    if "tok_emb.weight" not in cfg.quant.keep_float_name_patterns:
        return
    model_cfg.non_recurrent_mlp_hidden_bonus = model_cfg.d_model * 4
    model_cfg.tail_mlp_hidden_bonus_step = model_cfg.d_model * 2
...
class RecurrentGPT(nn.Module):
    def __init__(self, cfg: ModelConfig):
        ...
        tail_hidden_bonus_step = cfg.tail_mlp_hidden_bonus_step
        ...
        self.tail = nn.ModuleList(
            [
                TransformerBlock(
                    cfg,
                    num_adapter_slots=0,
                    mlp_hidden_bonus=non_recurrent_hidden_bonus + tail_idx * tail_hidden_bonus_step,
...
def validate_config(cfg: TrainConfig) -> None:
    ...
    if cfg.model.tail_mlp_hidden_bonus_step < 0:
        raise ConfigError("tail_mlp_hidden_bonus_step must be >= 0")
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    tilt_tail_mlp_width_toward_final_block_on_final_tail_q_small_batch_line(cfg)
    return cfg
