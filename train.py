...
def align_final_tail_mlp_train_and_export_precision_on_tied_embed_final_tail_q_line(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.tie_embeddings:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 1 or model_cfg.tail_layers != 3:
    ...
    cfg.quant.keep_float_name_patterns = tuple(
        dict.fromkeys((*cfg.quant.keep_float_name_patterns, *new_patterns))
    )
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    align_final_tail_mlp_train_and_export_precision_on_tied_embed_final_tail_q_line(cfg)
    return cfg
