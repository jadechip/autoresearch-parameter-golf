...
def reallocate_q_rank_into_high_precision_final_tail_block(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.tie_embeddings:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 1 or model_cfg.tail_layers != 3:
    ...
    if "tok_emb.weight" not in cfg.quant.keep_float_name_patterns:
        return
    high_precision_final_tail_patterns = (
        "tail.2.attn.q_proj",
        "tail.2.attn.out_proj",
        "tail.2.mlp.fc.weight",
        "tail.2.mlp.proj.weight",
    )
    if all(pattern in cfg.quant.keep_float_name_patterns for pattern in high_precision_final_tail_patterns):
        return
    model_cfg.q_low_rank = (model_cfg.d_model * 3) // 16
    cfg.quant.keep_float_name_patterns = (*cfg.quant.keep_float_name_patterns, *high_precision_final_tail_patterns)
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    reallocate_q_rank_into_high_precision_final_tail_block(cfg)
    return cfg
