...
def keep_first_tail_attention_float_on_four_block_near_cap_line(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.tie_embeddings or model_cfg.final_tail_smear_gate:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 0 or model_cfg.recurrence_loops != 0 or model_cfg.tail_layers != 4:
    ...
    if model_cfg.mlp_mult != 2 or model_cfg.shared_mlp_hidden_bonus != 0:
        return
    if model_cfg.non_recurrent_mlp_hidden_bonus != model_cfg.d_model * 5:
        return
    expected_tail_bonuses = (
        model_cfg.d_model * 6,
        model_cfg.d_model * 11 // 2,
    ...
    if not math.isclose(cfg.quant.clip_percentile, 96.5, rel_tol=0.0, abs_tol=1e-9):
        return
    cfg.quant.keep_float_name_patterns = (*cfg.quant.keep_float_name_patterns, "tail.0.attn.")
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    keep_first_tail_attention_float_on_four_block_near_cap_line(cfg)
    return cfg
