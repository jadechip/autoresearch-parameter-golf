...
def reallocate_recurrent_mlp_bonus_into_true_3x_tail_depth(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 2 or model_cfg.tail_layers != 7:
        return
    if model_cfg.mlp_mult != 2:
        return
    if model_cfg.shared_mlp_hidden_bonus != (model_cfg.d_model * 3) // 8:
        return
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    reallocate_recurrent_mlp_bonus_into_true_3x_tail_depth(cfg)
    return cfg
