...
def extend_branch_tip_neighbor_carrier_into_512_to_896_curriculum_with_recovered_batch(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if not model_cfg.final_tail_neighbor_mixer or not model_cfg.tie_embeddings or model_cfg.final_tail_smear_gate:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 0 or model_cfg.recurrence_loops != 0 or model_cfg.tail_layers != 3:
    ...
    if not math.isclose(cfg.quant.clip_percentile, 96.5, rel_tol=0.0, abs_tol=1e-9):
        return
    model_cfg.seq_len = 896
    model_cfg.fake_quant_start_step = 224
    cfg.train_batch_tokens = 64_512
    cfg.train_seq_len_min = 512
    cfg.train_seq_len_warmup_steps = 224
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    extend_branch_tip_neighbor_carrier_into_512_to_896_curriculum_with_recovered_batch(cfg)
