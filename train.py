...
def add_final_tail_canon_insert_on_near_cap_carrier(cfg: TrainConfig) -> None:
    model_cfg = cfg.model
    if model_cfg.final_tail_canon_rank != 0:
        return
    if model_cfg.final_tail_smear_gate or not model_cfg.tie_embeddings:
        return
    if model_cfg.stem_layers != 0 or model_cfg.shared_layers != 1 or model_cfg.recurrence_loops != 1 or model_cfg.tail_layers != 3:
        return
    if model_cfg.mlp_mult != 2 or model_cfg.shared_mlp_hidden_bonus != model_cfg.d_model:
    ...
    if not math.isclose(cfg.quant.clip_percentile, 96.5, rel_tol=0.0, abs_tol=1e-9):
        return
    model_cfg.final_tail_canon_rank = model_cfg.d_model // 8
...
class CanonMixer(nn.Module):
    def __init__(self, cfg: ModelConfig, rank: int):
        super().__init__()
        # Canon uses low-rank previous-token and prefix-mean features to stay cheaper than attention branches.
        self.down_proj = FakeQuantLinear(
            cfg.d_model,
            rank,
            bias=False,
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    add_final_tail_canon_insert_on_near_cap_carrier(cfg)
    return cfg
